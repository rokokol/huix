...
| `hyprlock-quote.sh` | диалог Моники для DDLC-локскрина (режимы `frame`/`lock`/`render`, `help` — справка): бокс+имя+реплика рендерятся ImageMagick'ом в один PNG (настоящая обводка, шрифт из системы через fc-match), image-виджет hyprlock перечитывает его раз в секунду, рендер в фоне; реплики топик за топиком, при локе первым идёт топик перезахода (`monika-reentry.txt`, по смене PID hyprlock); глитчи экрана+имени+текста одним механизмом — по Пуассону и на неверный пароль |
