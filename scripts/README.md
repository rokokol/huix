...
| `hyprlock-quote.sh` | диалог Моники для DDLC-локскрина (режимы `frame`/`name`, `help` — справка): побуквенная печать реплик топик за топиком с пиксельным переносом строк, при локе первым идёт топик перезахода (`monika-reentry.txt`, по смене PID hyprlock); глитчи экрана+имени+текста одним механизмом — по Пуассону и на неверный пароль |
