...
- **`skvpn.py` — единственный, кто едет в store, а не читается из чекаута**. Он ходит от root (пишет `/etc/sing-box`, дёргает systemd), а кто может писать в `$HUIX/scripts`, тот получил бы root через `sudo`. Поэтому его тело уходит в `writePython3Bin`, и правка применяется ребилдом, как у обычного модуля
