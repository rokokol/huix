...
- **Оба режима в конце подтягивают и `~/Projects`** — каждый репозиторий там перематывается вперёд и ничего больше: без ребейза, коммитов и пушей. Разошедшийся, грязный или оставшийся без upstream считается и не трогается, а файл `.nosync` в корне репозитория исключает его ещё до fetch. Отключается флагом `--no-projects`
