...
| два пальца в движении      | колесо мыши под пальцами: скролл в kitty и rofi, крутилки waybar |
...
Плагин на каждом касании ставит указатель под пальцы, и на этом стоит колесо: живой жест переводит дельту свайпа обратно в пиксели и на каждые 24 px шлёт `wlrctl pointer scroll` через виртуальный указатель, асинхронно (`hl.exec_cmd`) — синхронный запуск Wayland-клиента изнутри компоситора ждал бы компоситор вечно. Два пальца, задержанные на месте до движения, — это перетаскивание окна, а не скролл. Приложения с `wl_touch` (Firefox, GTK, Qt) сначала получают касание, потом отмену и уже колесо — на старте свайпа возможен небольшой двойной сдвиг. ПКМ двумя пальцами по-прежнему нет: кнопок плагин не эмулирует, контекстное меню по долгому нажатию рисуют сами приложения. У rofi и kitty на Wayland `wl_touch` нет вовсе — тап в них не работает, пока их не пропатчить, а колесо теперь работает
