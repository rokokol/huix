...
| `services/hyprlock.nix` | DDLC-локскрин: фон just_monika, диалог как в игре — `hyprlock-quote.sh` рендерит бокс+имя+реплику одним PNG через ImageMagick (настоящая обводка, кадр раз в секунду, рендер в фоне), первая реплика при локе — про «перезаход в игру», имя и текст глитчатся вместе с экраном (по Пуассону и на неверный пароль), индикатор раскладки у поля ввода, сердечки вместо точек пароля; все пути через `huixDir`, кадр и кэш подложки — в `~/.cache/huix` |
