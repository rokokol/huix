...
| от левого/правого края вбок | соседний воркспейс, тянется за пальцем (встроенный `gestures.workspace_swipe_touch`, полоса `gaps_out` + рамка); хватает 10 % ширины экрана |
| вдоль левого края          | яркость по 5 % на 20 px, вверх — ярче (OSD от swayosd)      |
| вдоль правого края         | громкость, так же                                           |
| щипок тремя                | живая лупа с якорем под пальцами                            |
...
Плагин на каждом касании ставит указатель под пальцы — на этом стоит якорь лупы. Слайдеры краёв — живой жест: дельта свайпа переводится обратно в пиксели, шаги копятся и уходят одним относительным вызовом `swayosd-client` не чаще раза в 60 мс, остаток — при отпускании. Каждый `hl.exec_cmd` форкает сам компоситор и запускает клиент на десятки миллисекунд, так что вызов на каждый шаг подтормаживал экран и дёргал уровень: вызовы шли внахлёст и завершались не по порядку. Процесс `hl.exec_cmd` не ждёт — клиент, запущенный синхронно изнутри компоситора, ждал бы компоситор вечно; ширина зоны — `edge_margin`, а до первого шага палец проходит порог распознавания жеста, 150 / `sensitivity` px. Скролл плагин не эмулирует: приложения с `wl_touch` скроллят одним пальцем сами, а колесо под пальцами только мешало им. Крутилки waybar пальцу недоступны: бар получает саму серию касаний и, пока палец лежит, на колесо не отвечает — потому яркость и громкость на краях. ПКМ двумя пальцами нет: кнопок плагин не эмулирует, контекстное меню по долгому нажатию рисуют сами приложения. У rofi 2.0.0 и kitty на Wayland `wl_touch` нет вовсе; rofi здесь собран с ветки `next` (`overlay-rofi`, почему и до какого релиза — в [WORKAROUNDS.md](../../../WORKAROUNDS.md)): тап выбирает, второй тап принимает, палец листает, тап мимо окна закрывает. kitty пропатчен на ноутбуке (`overlay-kitty`): тап — клик, палец сразу в движении — прокрутка с инерцией, палец, подержанный 0,4 с и потом ведомый, — выделение
...
- **заголовки окон только в tablet-режиме** (`services/titlebars.nix`) — hyprbars с кнопками закрыть и развернуть (maximize, не fullscreen: полноэкранное окно накрывает свой же бар, и палец не вернулся бы), глифы Nerd Font из DepartureMono, цвета из палитры. Иконки hyprbars рисует жёстким `sans`, поэтому плагин пропатчен на `bar_text_font` — [WORKAROUNDS.md](../../../WORKAROUNDS.md). Бар есть у всех окон, и у тех, что рисуют свою кнопку закрытия: палец находит одни и те же кнопки в одном месте. Перетаскивание за бар работает пальцем из коробки; `dwindle.precise_mouse_move` кладёт брошенное окно на ту сторону цели, где курсор, а не только в зазоры
