...
| `services/lid-mode.nix`           | ноутбучный тумблер `SUPER SHIFT+A` и `bindl switch:` на крышку — режим "крышка гасит экран, а не усыпляет" (`lid-mode.sh`); включается опцией `custom.hyprland.lidNoSleep`                                                                                                                                          |
