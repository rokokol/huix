...
| два пальца в движении      | колесо мыши под пальцами: скролл в kitty и rofi             |
| вдоль левого края          | яркость, вверх — ярче (OSD от swayosd)                      |
...
Плагин на каждом касании ставит указатель под пальцы, и на этом стоит колесо: живой жест переводит дельту свайпа обратно в пиксели, по первому шагу запирает ось (косой палец крутит одно) и на каждые 24 px шлёт `wlrctl pointer scroll` через виртуальный указатель, асинхронно (`hl.exec_cmd`) — синхронный запуск Wayland-клиента изнутри компоситора ждал бы компоситор вечно. До крутилок waybar колесо так не добирается: бар получает и саму серию касаний, и, пока палец лежит, на колесо не отвечает — поэтому яркость и громкость живут на краях экрана тем же механизмом шагов, через `swayosd-client`, и OSD показывает уровень. Два пальца, задержанные на месте до движения, — это перетаскивание окна, а не скролл. Приложения с `wl_touch` (Firefox, GTK, Qt) сначала получают касание, потом отмену и уже колесо — на старте свайпа возможен небольшой двойной сдвиг. ПКМ двумя пальцами по-прежнему нет: кнопок плагин не эмулирует, контекстное меню по долгому нажатию рисуют сами приложения. У rofi и kitty на Wayland `wl_touch` нет вовсе — тап в них не работает, пока их не пропатчить, а колесо теперь работает
