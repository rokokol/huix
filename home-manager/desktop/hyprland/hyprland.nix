    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        input = {
          ...
          # Тачпад шлёт непрерывный (пальцевой) скролл, а не дискретные «щелчки»,
          # поэтому бинды на mouse_up/mouse_down (зум SUPER+ALT+колесо) по нему не
          # срабатывают. 2 = эмулировать дискретный скролл в т.ч. для тачпада —
          # так двупальцевый скролл начинает триггерить эти бинды. На мыши
          # (дискретный скролл и так есть) ничего не меняет, поэтому безопасно
          # держать это общим для обоих хостов.
