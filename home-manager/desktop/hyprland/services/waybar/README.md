...
- **Новый компонент** = новый файл рядом + опция `custom.waybar.<фича>` + место в `modules-right` в `bar.nix` + селектор в списке островов. Не забыть `git add` (sync берёт только отслеживаемое)
