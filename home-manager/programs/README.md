...
| --- | --- |
| `scripts.nix` | обёртки над [`scripts/`](../../scripts/README.md) — сейчас один будильник: кладёт зависимости в PATH и звук из freedesktop-темы (файл живёт в `scripts/`, импортируется отсюда) |
...
| `virtual-mic.nix` | шов к флейку [virtual-media-devices](https://github.com/rokokol/virtual-media-devices): включает виртуальный микрофон |
| `zen.nix` | браузер Zen с модами и поисковыми движками |

## Тонкости

- `virtual-mic.nix` — одна строка: команда и её зависимости приезжают из флейка [virtual-media-devices](https://github.com/rokokol/virtual-media-devices), камерная половина того же флейка включается системным слоем в `nixos/configuration-pc.nix` (ей нужен модуль ядра)
