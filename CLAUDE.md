...
- **Both modes then sweep `~/Projects`**, fast-forwarding every repository there and nothing else. The sweep runs from a trap on EXIT, because huix leaves through five different exits and one of them would eventually be forgotten; fetching fans out through `xargs -P 8` with a timeout each, since the repositories are independent and a dead remote must not hold the login hostage. A repository that has no upstream, has diverged or carries a `.nosync` file is left alone — `.nosync` is the opt-out for a clone whose fetch is not worth the login, such as a nixpkgs checkout. `PROJECTS_DIR` moves the directory, `--no-projects` skips the sweep
