...
- **Always include the target host in commit messages** when a change is host-specific. Prefix the subject with `[pc]` or `[laptop]` (for example: `[pc] enable CUDA cache` or `[laptop] add lid mode toggle`). Use `[both]` (or omit the prefix) only when the change genuinely affects both hosts
