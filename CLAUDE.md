...
- **After a change that is meant to leave behaviour alone, run `nix run .#drv-diff`** — a rename, a `with pkgs`, an `inherit`, a comment, a move between files. It evaluates both hosts, every check and the formatter here and at `HEAD`, and names anything whose derivation path moved. A move it reports is either a bug you just wrote or a dependency you did not know the derivation had; both are worth a minute. `-r REV` compares against something else
...
- **Both modes then sweep `~/Projects` and the shared Claude skills**, catching every repository there up to its upstream: a fast-forward where there are no local commits, a `rebase --autostash` where there are. A conflicting rebase is wound all the way back with `rebase --abort`, because a half-applied rebase left behind by a background job at login is worse than an out-of-date checkout. It never commits and never pushes. The sweep runs from a trap on EXIT, because huix leaves through five different exits and one of them would eventually be forgotten; fetching fans out through `xargs -P 8` with a timeout each, since the repositories are independent and a dead remote must not hold the login hostage — three of the checkouts here point at repositories that no longer exist. The opt-out is a `nosync` file **inside `.git/`**, where it needs no `.gitignore` and shows in no `git status`; it is the answer for a clone whose fetch is not worth the login, and it sits in the nixpkgs checkout and the read-only forks. Neither directory is guessed, and neither is spelled twice: `PROJECTS_DIR` comes from `projectsDir` in `commonArgs` and `SKILLS_DIR` from `programs.claude-account.sharedDir`, which the seam in `home-manager/programs/cli/claude.nix` names because the module leaves it null. Both are `home.sessionVariables`, so a hand call of `syssync` sweeps what the unit sweeps; `home-manager/desktop/sync.nix` reads them back out of `config.home.sessionVariables` into the unit's Environment, beside `HUIX`, because a user unit inherits no session variable. The script's `$HOME/Projects` fallback is only for a shell that has neither. `--no-projects` skips the sweep
...
- **Third-party flakes must reuse the system `nixpkgs` and `home-manager` via `follows`** — otherwise the input pulls its own copy of nixpkgs, duplicating packages and downloads. The `ddlc-*` inputs additionally need `inputs.ddlc-palette.follows`, or the colours diverge silently (see `DEVIATIONS.md`)
- **The myWiki vault is `myWikiDir` = `$HOME/myWiki` on both hosts**, and it arrives through `commonArgs` rather than being built out of anything host-specific. The path must stay host-invariant: Syncthing carries symlinks verbatim, so anything pointing into the vault would dangle on the other host if the hosts disagreed
...
- **Binary cache substituters are declared per-module** — the shared ones in `nixos/services/system/cachix.nix`, a host-specific one next to what needs it (`nixos/pc/nvidia.nix` → the CUDA cache, which is not a Cachix one). When adding a heavy build, prefer adding a substituter over rebuilding
...
- **Screen zoom differs by input device.** `scripts/zoom.sh` is bound to `ALT+WIN+wheel`, but Hyprland `mouse_up`/`mouse_down` binds fire only for a physical wheel, never a touchpad — no input option changes this. The laptop therefore has a separate touchpad `hl.gesture` pinch → `cursor_zoom` (the gesture engine has no generic `exec`, so it cannot call the script), and the touchscreen a three-finger pinch through hyprgrass in `services/touch-gestures.nix`. Three mechanisms, same feel; keep all
...
- **Workarounds that exist only because of an upstream bug live in `WORKAROUNDS.md`**, one entry each with a mechanical removal check. Cross-cutting choices that differ from the obvious arrangement live in `DEVIATIONS.md`. Check both before "cleaning up" a line that looks pointless

## Extracted flakes

Eleven pieces that outgrew the config live in their own repos (`rokokol/hyprland-screen-shader`, `claude-account`, `rofi-wooordhunt`, `virtual-media-devices`, `skvpn`, and the `ddlc-*` family) and come back as flake inputs. Each keeps exactly **one seam** here — a file that enables the module and supplies what it ships none of, nothing more. Before adding an option to a seam, read the comment at its top: each records what the module already owns, because every one of them has been broken once by adding such a line back. The rest is the module's own README
