...
- **No trailing period at the end of a paragraph.** Periods separate sentences *within* a paragraph, but the final sentence of a paragraph — and any standalone one-liner (a single-line comment, a list item, a notification, a commit-message line) — gets no closing period. Other terminal punctuation (`?`, `!`, `…`, `:`) is unaffected; an inner sentence still ends with its period before the next one begins. Applies everywhere: comments, `README.md`, issues, UI strings, help text, commit messages
- **Quotes are always straight `"…"` — never `«ёлочки»`, `„лапки“` or curly `“…”`**, in every language and every file type: comments, `usage()`/help text, READMEs, notification and UI strings, commit messages. When a quote lands *inside* a string literal, escape it (`"toggle \"lid doesn't sleep\" (laptop only)"` in Nix, `echo "microphone \"$name\""` in bash) — never leave a bare `"` that would terminate the string. Anything mechanical here (a repo-wide replace) must be byte-wise (`\xc2\xab`/`\xc2\xbb`) and check the string-literal cases by hand afterwards
- **Keep comments terse.** A comment states the non-obvious *why* in as few words as possible — ideally one line. Don't stack multi-line explanations where a short clause does; if a comment needs a paragraph, the code probably needs restructuring instead
...
- **Always include the current host in commit messages**. Prefix the subject with `[pc]` or `[laptop]` (for example: `[pc] enable CUDA cache` or `[laptop] add lid mode toggle`)
