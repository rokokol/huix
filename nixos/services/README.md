...
Всё биндится на `127.0.0.1`, наружу не торчит ничего — firewall под эти сервисы не открыт. Исключение одно, Tailscale: у него открыт UDP 41641 и доверен интерфейс `tailscale0`, чтобы ноды тайлнета доставали хост напрямую, а не через DERP-релей. Какие ноды куда достают, решает политика тайлнета в админке Tailscale, а не huix

sshd (`system/sshd.nix`) слушает на всех интерфейсах, но порт 22 в firewall не открыт, поэтому попасть на него можно только через доверенный `tailscale0`. Публичные ключи хостов перечислены в модуле
...
- `smartd` (`system/smartd.nix`) следит за всеми дисками, которые находит при загрузке. Длинный self-test идёт по субботам и воскресеньям с 19:00: самому медленному диску, SATA-диску ноута, нужно 85 минут, так что тест кончается до 21:00. Короткий идёт каждый день в 14:00 и по будням в 20:00, чтобы не пересекаться с длинным. Если час теста прошёл, пока хост спал или был выключен, `smartd` запускает тест при первой проверке после пробуждения или загрузки. После выключения это помнит файл состояния в `/var/lib/smartd`. О проблеме `smartd` сообщает уведомлением на рабочий стол и сообщением `wall` в терминалы. Ручная проверка: `sudo smartctl -x /dev/sda`, журнал: `journalctl -u smartd`
...
- `devices/sensors.nix` (`rokokol.sensors.enable`, ноутбук) — `iio-sensor-proxy` над IIO-датчиками ядра: акселерометр для автоповорота экрана в tablet-режиме, `monitor-sensor` для скриптов. Демон поднимается по D-Bus, а udev-теги на датчики встают только при перечислении устройств — после первого включения нужна перезагрузка, не перелогин
