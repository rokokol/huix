...
- Повседневный VPN — sing-box, профили переключаются шаблонным юнитом: `sudo skvpn up <профиль>`. Общая часть (TUN, DNS, маршрутизация) лежит в `/etc/sing-box/base.d` из nix, а профиль это один аутбаунд с тегом `proxy` в `/etc/sing-box/profiles`, куда его кладёт `skvpn` из подписки. В TUN исключён диапазон тайлнета, иначе Tailscale недостижим при поднятом туннеле, а российские адреса и зоны уходят в `direct` мимо туннеля. Выбранный профиль запоминается в `/var/lib/skvpn/active` и поднимается обратно юнитом `skvpn-restore`: `systemctl enable` тут не годится, каталог юнитов — симлинк в store
